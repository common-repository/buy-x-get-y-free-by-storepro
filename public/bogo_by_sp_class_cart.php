...
class bogo_by_sp_public_class {
    ...
    Public function __construct() {
        ...
        if ($global_enable === 1) {
            ...
            add_filter('woocommerce_get_price_html', array(__CLASS__, 'bogo_by_sp_cart_icon_to_price'), 10, 2);
            ...
            add_action('woocommerce_before_add_to_cart_button', array(__CLASS__, 'bogo_related_products_free_icon'));
            ...
            if( ($main_product_id != 0) && ($main_product_qty != 0) && ($free_product_id != 0) && ($free_product_qty != 0)  ){
                ...
                if($main_product_id == $product_id){ 
                    ...
                    if (!$free_added && ($cart_main_qty >= $main_product_qty)) {
                        ...
                        // change 1
                        $free_prod_key = WC()->cart->add_to_cart($free_product_id, $cart_main_qty, 0, array(), $cart_item_data);
                    } 
            ...
            if( ($main_product_2_id != 0) && ($main_product_2_qty != 0) && ($free_product_2_id != 0) && ($free_product_2_qty != 0)  ){
                ...
                if($main_product_2_id == $product_id){ 
                    ...
                    if (!$free_added_2 && ($cart_main_qty_2 >= $main_product_2_qty)) {
                        ...
                        $free_prod_key_2 = WC()->cart->add_to_cart($free_product_2_id, $cart_main_qty_2, 0, array(), $cart_item_data);
            ...
            if( ($main_product_3_id != 0) && ($main_product_3_qty != 0) && ($free_product_3_id != 0) && ($free_product_3_qty != 0)  ){
                ...
                if($main_product_3_id == $product_id){ 
                    ...
                    if (!$free_added_3 && ($cart_main_qty_3 >= $main_product_3_qty)) {
                        ...
                        // change 3
                        $free_prod_key_3 = WC()->cart->add_to_cart($free_product_3_id, $cart_main_qty_3, 0, array(), $cart_item_data);
                    } 
...
public static function bogo_by_sp_cart_remove($cart_item_key)
    {
        static $is_processing = false; // Prevent recursion
        if ($is_processing) {
            return;
        }
        ...
        $main_product_ids = [
            (int) get_option('sp_bogo_product_buy'), // Main product 1 ID
            (int) get_option('sp_bogo_product_2_buy'), // Main product 2 ID
            (int) get_option('sp_bogo_product_3_buy') // Main product 3 ID
        ];

        // Determine which main product was removed to identify the correct free product flag to check
        $removed_product_id = null;
        $cart = WC()->cart->get_cart();
        foreach ($cart as $key => $cart_item) {
            if ($key == $cart_item_key) {
                $removed_product_id = $cart_item['product_id'];
                break; // Found the removed product
            }
        }

        // If a main product was removed, find its associated free product
        if (in_array($removed_product_id, $main_product_ids)) {
            $free_product_flag = ''; // Determine which free product flag to check based on the removed product ID
            if ($removed_product_id == $main_product_ids[0]) {
                $free_product_flag = 'free_product';
            } elseif ($removed_product_id == $main_product_ids[1]) {
                $free_product_flag = 'free_product_2';
            } elseif ($removed_product_id == $main_product_ids[2]) {
                $free_product_flag = 'free_product_3';
            }

            // Iterate over the cart to find and remove the associated free product based on the determined flag
            foreach ($cart as $key => $cart_item) {
                if (!empty($cart_item[$free_product_flag]) && $cart_item[$free_product_flag] === true) {
                    WC()->cart->remove_cart_item($key); // Remove the associated free product
                    break; // Assuming only one free product is associated with each main product
                }
            }
        }

        $is_processing = false; // Reset the flag
    }
    ...
    Public static function bogo_by_sp_cart_icon_to_price($price, $product) {
        ...
        // Additional condition to prevent modification on the single product page
        if (is_product()) {
            return $price; // Do not modify the price on the single product page
        }
        ...
        if (($product_id == $free_product_id && empty($free_product_ids)) ||
            ($product_id == $free_product_2_id && empty($free_product_ids)) ||
            ($product_id == $free_product_3_id && empty($free_product_ids))) {
            return '<span class="price">'. $price .' </span>';
        } elseif (($product_id == $free_product_id && !empty($free_product_ids)) ||
                  ($product_id == $free_product_2_id && !empty($free_product_ids)) ||
                  ($product_id == $free_product_3_id && !empty($free_product_ids))) {
